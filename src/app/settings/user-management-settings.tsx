...
export default function UserManagementSettings() { // Renamed component
  ...
  const handleAddUser = async (data: UserFormValues) => {
    ...
    if (window.electronAPI) {
      try {
        const newUserId = await window.electronAPI.createUser(data);
        if (newUserId) {
  ...
  return (
    <div className="space-y-6">
      {/* Add New User Button - Moved here from PageHeader */}
