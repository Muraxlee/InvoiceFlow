...
export function InvoiceForm({ onSubmit, defaultValues: defaultValuesProp, isLoading }: InvoiceFormProps) {
  const form = useForm<InvoiceFormValues>({
    ...
    defaultValues: defaultValuesProp || {
      ...
      customerEmail: "",
      customerAddress: "",
      invoiceNumber: "", // Will be auto-generated by useEffect
      notes: "",
      termsAndConditions: "",
      invoiceImage: "",
    },
  ...
  useEffect(() => {
    ...
    if (isCreatingNew) {
      if (form.getValues('invoiceDate') === null) {
        form.setValue("invoiceDate", new Date(), { shouldValidate: false });
      }
      if (form.getValues('dueDate') === null) {
        ...
        form.setValue("dueDate", thirtyDaysFromNow, { shouldValidate: false });
      }

      const currentInvoiceNumber = form.getValues('invoiceNumber');
      const invoiceDateForNumber = form.getValues('invoiceDate') || new Date(); 
      
      // Only generate if currentInvoiceNumber is empty or an old placeholder
      // This ensures it's generated once for new invoices.
      if (!currentInvoiceNumber || currentInvoiceNumber.startsWith("INV-")) {
        form.setValue("invoiceNumber", generateInvoiceNumber(invoiceDateForNumber), { shouldValidate: true });
      }
    }
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="flex justify-end gap-2">
          <Button type="button" variant="outline" onClick={() => form.reset(defaultValuesProp || { 
            ...
            customerEmail: "",
            customerAddress: "",
            invoiceNumber: generateInvoiceNumber(new Date()), // Regenerate on reset for new form
            notes: "",
            termsAndConditions: "",
            invoiceImage: "",
